<div align="center">
  <img src="assets/imagemDeFundo.png" width="1000px" height="370px" style="border-radius: 20px; box-shadow: 0px 0px 40px #050a30;"/>
  
  <br>

 <a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.herokuapp.com?font=Press+Start+2P&weight=400&size=18&pause=1000&color=f0f8ff&center=true&vCenter=true&width=700&height=60&separator=|&lines=System.out.println(%22Hello+World!%22);|Construindo+soluções+com+Java|Explorando+Dados+%26+Automação|Desenvolvendo+com+Spring+Boot|Transformando+Café+em+Código!" alt="Typing SVG" />
</a>
</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=4D96FF&height=2&section=footer" width="100%"/>
</div>

<br>

<table width="100%" cellpadding="10"
       style="border:none; border-collapse:collapse;">
  <tr>
  <td width="60%" valign="top" style="padding:12px; border:none;">
    <h3>⚡ Deixa eu me apresentar</h3>
    <p align="left">
      Sou estudante de <b>Sistemas de Informação</b> e atualmente atuo como
      estagiário no <b>Ministério das Comunicações</b>.
      Tenho interesse principalmente em <b>desenvolvimento Backend,
      dados e automação</b>, utilizando Java, Spring Boot, Python e SQL
      para transformar problemas reais em soluções práticas.
      <br><br>
      Ao longo da graduação, venho desenvolvendo projetos envolvendo
      <b>APIs REST, microsserviços, bancos de dados, inteligência artificial,
      análise de dados e desenvolvimento web</b>.
      Gosto de entender como as coisas funcionam por trás do código e
      transformar conhecimento em projetos que possam ser utilizados
      na prática.
      <br><br>
      Estou constantemente buscando evoluir, aprender novas tecnologias
      e construir uma base sólida para minha carreira em tecnologia.
    </p>
  </td>
    <td width="50%" valign="top" align="center" style="border:none;">
      <h3>🛠️ Arsenal Técnico</h3>
      <div align="center">
         <img
      src="https://skillicons.dev/icons?i=java,spring,maven,gradle,python,mysql,git,docker,postman,idea,javascript,html,css,angular,vscode,php&perline=4"
      height="300"  width="100%"
      alt="Arsenal Técnico" />
      </div>
    </td>
  </tr>
</table>


<br>

<div align="center" border="10">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/MateusLima909/MateusLima909/output/pacman-contribution-graph-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/MateusLima909/MateusLima909/output/pacman-contribution-graph.svg">
    <img alt="Generated Pacman" src="https://raw.githubusercontent.com/MateusLima909/MateusLima909/output/pacman-contribution-graph-dark.svg" width="100%"/>
  </picture>
</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=4D96FF&height=2&section=footer" width="100%"/>
</div>

<br>

<table width="100%" cellpadding="10" style="border-collapse:collapse; border:0;">
  <tr style="border:0;">
    <td width="40%" valign="middle" style="border:0; background-color:#0d1117;">
      <img src="assets/ondaDoMar.png" width="360px" height="800px"
           style="border:0; background-color:#0d1117;" />
    </td>
    <td width="70%" style="border:none;">
      <h3>🚀 O que estou fazendo agora?</h3>
      <br>
      <p>
        🏛 <b>Atualmente:</b> Estagiário no <b>Ministério das Comunicações</b>,
        atuando com processos, organização e restrição de dados. (Tá seguro)
        <br><br>
        ☕ <b>Foco Técnico:</b> Aprofundando meus conhecimentos em
        <b>Java, Spring Boot, SQL e Arquitetura de Software</b>.
        <br><br>
        📊 <b>Dados & Automação:</b> Desenvolvendo soluções com
        <b>Python, Power BI e APIs</b> para transformar dados em informações
        úteis e automatizar tarefas.
        <br><br>
        🤖 <b>Explorando:</b> Inteligência Artificial, NLP e aplicações
        de LLMs em projetos práticos.
        <br><br>
        🥁 <b>Hobby:</b> Baterista e guitarrista nas horas vagas
        (somente o básico).
      </p>
    </td>
  </tr>
</table>

<br>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=4D96FF&height=2&section=footer" width="100%"/>

  <h3>🏆 Projetos em Destaque</h3>
  <table align="center">
    <p align="center">
      <picture>
        <source media="(prefers-color-scheme: light)" srcset="https://www.gitskins.com/api/section/projects?username=MateusLima909&theme=neon&mode=light" />
        <img width="100%" src="https://www.gitskins.com/api/section/projects?username=MateusLima909&theme=neon" alt="MateusLima909 projects section" />
      </picture>
    </p>
  </table>

  <br>
  <a href="https://www.linkedin.com/in/mateuslima-santos"> 
    <img src="https://img.shields.io/badge/linkedin-4D96FF?style=for-the-badge&logo=linkedin&logoColor=white" /> 
  </a> 
  <a href="mailto:mateuslimasantos909@gmail.com"> 
    <img src="https://img.shields.io/badge/Gmail-4D96FF?style=for-the-badge&logo=gmail&logoColor=white" /> 
  </a>

  <div>
    <img style="100%" width="100%"src="https://capsule-render.vercel.app/api?type=waving&height=150&section=footer&reversal=false&fontSize=70&fontColor=FFFFFF&fontAlign=50&fontAlignY=50&stroke=-&descSize=20&descAlign=50&descAlignY=50&theme=tokyonight"  />
  </div>
</div>
